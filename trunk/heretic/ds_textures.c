...
#define TEXTURE_BLOCK_SIZE3 (128*128)
...
dstex_t ds_blank_tex = {66666,8,8,8,8,3,0};
...
{
	host_basepal = W_CacheLumpName ("PLAYPAL",PU_STATIC);
#ifdef WIN32
...
{
#ifdef ARM9
	if(addr >= (byte *)VRAM_A && addr < VRAM_B)
	{
		vramSetBankA(VRAM_A_TEXTURE);
		return;
	}
	if(addr >= (byte *)VRAM_B && addr < VRAM_C)
	{
		vramSetBankB(VRAM_B_TEXTURE);
		return;
	}
	if(addr >= (byte *)VRAM_C && addr < VRAM_D)
	{
		vramSetBankC(VRAM_C_TEXTURE);
		return;
	}
	if(addr >= (byte *)VRAM_D && addr < VRAM_E)
	{
