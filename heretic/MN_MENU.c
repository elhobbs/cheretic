...
static void hSetMenu(MenuType_t menu);
				...
				if(askforquit)
				{
					switch(typeofask)
					{
						...
						case 2:
							...
							askforquit = false;
							paused = false;
							I_SetPalette(host_basepal);//W_CacheLumpName("PLAYPAL", PU_CACHE));
	...
	if(MenuActive == false && !chatmodeon)
	{
		switch(key)
		{
			...
			case KEY_F11: // F11 - gamma mode correction
				...
				I_SetPalette(host_basepal);//(byte *)W_CacheLumpName("PLAYPAL", PU_CACHE));
				return true;
#endif
		}
