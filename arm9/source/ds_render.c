...
int viewwidth = DS_SCREEN_WIDTH;
int viewheight = DS_SCREEN_HEIGHT;
int viewheightfrac;
int viewwindowx = 0;
int viewwindowy = 0;
...
char	occlusion[DS_SCREEN_WIDTH+2];	// +2 for guard columns to avoid clamping
...
#ifdef ARM9
#define OU(w,seg) (((float)((seg)->sidedef->textureoffset+(seg)->offset)/(float)FRACUNIT))
#define OV(w,seg) (((float)((seg)->sidedef->rowoffset)/(float)FRACUNIT))
#define OV_PEG(w,seg,v_offset) (OV((w),(seg))-(((float)(v_offset)/(float)FRACUNIT)))
#else
...
(peg)?\
(\
(w).vb=OV((w),(seg))+((float)textures_ds[(w).name].height),\
(w).vt=((w).vb-((float)(lineheight)))\
):(\
(w).vt=OV((w),(seg))+(0.0f),\
(w).vb=OV((w),(seg))+((float)(lineheight))\
)
...
(peg)?\
(\
(w).vb=OV_PEG((w),(seg),(v_offset))+((float)textures_ds[(w).name].height),\
(w).vt=((w).vb-((float)(lineheight)))\
):(\
(w).vt=OV((w),(seg))+(0.0f),\
(w).vb=OV((w),(seg))+((float)(lineheight))\
...
(peg)?\
(\
(w).vb=OV_PEG((w),(seg),(v_offset))+((float)textures_ds[(w).name].height/(float)textures[(w).name]->height),\
(w).vt=((w).vb-((float)(lineheight)/(float)textures[(w).name]->height))\
):(\
(w).vt=OV((w),(seg))+(0.0f),\
(w).vb=OV((w),(seg))+((float)(lineheight)/(float)textures[(w).name]->height)\
	...
	else /* twosided */
	{
		...
		if (/*temptex && */seg->sidedef->midtexture != 0)
		{
			 ...
			 );*/
			{
				...
				wall.ur=OU(wall,(seg))+(((seg->length*textures_ds[wall.name].width_scale)/(float)FRACUNIT));
...
#ifdef ARM9
				wall.vb *= (float)textures_ds[wall.name].height;
				wall.vt *= (float)textures_ds[wall.name].height;
#endif
...
boolean TransformAndClipSegment( float v[2][2], float ends[2] ) {
	...
	clip[0][0] = f32tofloat(PosTestXresult());
	clip[0][3] = f32tofloat(PosTestWresult());
	
	PosTest((short)v1[0], 0,(short)v1[1]);
	clip[1][0] = f32tofloat(PosTestXresult());
	clip[1][3] = f32tofloat(PosTestWresult());
...
void IR_RenderWalls(void) {
	...
	texSort_t	*wallSort = alloca( sizeof( wallSort[0] ) * num_gl_walls );
	...
	glBegin(GL_QUADS);
		...
		xyz[1] = wall->ytop;
		xyz[2] = MAP_COORD(wall->side->sideSeg.v1->y);
		lightInt = (name == skytexture ? 0xffffff : FadedLighting( xyz[0], xyz[2], wall->light ));
		glColor3ubv((GLubyte *)&lightInt);
		glTexCoord2fv(st);
		glVertex3fv(xyz);
...
void IR_RenderPlayerView (player_t* player) {
...
#if 0
    viewx = 123<<16;//player->mo->x;
    viewy = -451<<16;//player->mo->y;
    viewz = 49<<16;//player->viewz;
#else
    viewx = player->mo->x;
    viewy = player->mo->y;
    viewz = player->viewz;
	...
	*/
	...
	//cover the screen with a tranclucent layer
	//if (skyIsVisible ) 
	{
		float	s;
		float	y;
	...
	for (i=0 ; i < numSectorPlanes ; i++ ) {
		...
		while(floor)
		{
			...
			for(j=2;j<count;j++)
			{
				...
				glTexCoord2f(p1[0]*2.0f,p1[2]*2.0f);
				glVertex3fv(p1);

				glTexCoord2f(p2[0]*2.0f,p2[2]*2.0f);
				glVertex3fv(p2);
#endif
	...
	/*printf("view: %f %f %f\n",
		...
		player->mo->y/65536.0f,
		player->viewz/65536.0f);*/
